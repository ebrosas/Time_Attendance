...
*	21/08/2022		Ervin		1.6			Refactored the logic in determining whether missing in or out
*	27/08/2022		Ervin		1.7			Added condition to check if RemarkCode = 'A'
*********************************************************************************************************************************************************************************/
	...
	SELECT	a.AutoId,
			...
			(	--Rev. #1.6
				CASE WHEN swp.One IS NULL AND a.IsLastRow = 1 AND ((a.dtIN IS NULL AND a.dtOUT IS NOT NULL) OR (a.dtIN IS NOT NULL AND a.dtOUT IS NULL)) THEN 'Missing Swipe IN or OUT' + CHAR(10) + CHAR(13) 
					WHEN swp.One IS NULL AND a.IsLastRow = 1 AND a.dtIN IS NULL AND a.dtOUT IS NULL AND RTRIM(a.RemarkCode) <> 'A' THEN 'Missing Swipe IN and OUT' + CHAR(10) + CHAR(13)	--Rev. #1.7
				ELSE '' END
			) +
