...
*	Revision History
*
*	Name: tas.Pr_GetAssignedOvertimeRequest_V2
*	Description: Get the currently assigned overtime requisitions
*
*	Date			Author		Revision No.	Comments:
*	18/03/2018		Ervin		1.0				Created
*	20/05/2018		Ervin		1.1				Added "IsOTRamadanExceedLimit" field in all load types
**********************************************************************************************************************************************************************************************************/

ALTER PROCEDURE tas.Pr_GetAssignedOvertimeRequest_V2
(   
...
AS
	...
	SELECT	DISTINCT 
			...
			CASE WHEN c.IsRamadan = 1 AND c.isMuslim = 1 AND RTRIM(c.ShiftCode) <> 'O' 
				AND (CASE WHEN a.OTApproved = 'N' THEN 0 ELSE DATEDIFF(n, d.OTStartTime, d.OTEndTime) END) > 120
				THEN 1
				ELSE 0
			END AS IsOTRamadanExceedLimit	--Rev. #1.1		
...
PARAMETERS:
	...
	EXEC tas.Pr_GetAssignedOvertimeRequest_V2 10001988, 1, 0, '', '', ''
	EXEC tas.Pr_GetAssignedOvertimeRequest_V2 10001988, 1, 0, '', '', '3320,3470'
	EXEC tas.Pr_GetAssignedOvertimeRequest_V2 10003632, 1			--Currently assigned to me
	EXEC tas.Pr_GetAssignedOvertimeRequest_V2 10001988, 1			--Currently assigned to all	
	EXEC tas.Pr_GetAssignedOvertimeRequest_V2 10003656, 2			--Currently assigned to others
	EXEC tas.Pr_GetAssignedOvertimeRequest_V2 0, 1, 0, '05/17/2018', '05/20/2018'
