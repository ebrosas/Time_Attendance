...
*	Revision History
*
*	Name: tas.Pr_GetEmpWithLongUnpaidLeave
*	Description: Retrieves the list of employees that have long period of unpaid leave 
*
*	Date:			Author:		Rev. #:		Comments:
*	28/04/2019		Ervin		1.0			Created
*	15/05/2019		Ervin		1.1			Implemented logic for Annual Leave, Sick Leave Paid, and Injury Leave
*	02/07/2019		Ervin		1.2			Added filter condition that exclude attendance records that have timesheet correction code
*******************************************************************************************************************************************************/

ALTER PROCEDURE tas.Pr_GetEmpWithLongUnpaidLeave
(
	@startDate		DATETIME,
	@endDate		DATETIME,
	@empNo			INT,
...
AS
	...
	WHERE a.DT BETWEEN @startDate AND @endDate
		...
		AND a.IsLastRow = 1
		AND ISNULL(a.CorrectionCode, '') = ''	--Rev. #1.2
		AND (a.EmpNo = @empNo OR @empNo IS NULL)
		AND (RTRIM(a.BusinessUnit) = @costCenter OR @costCenter IS NULL)
	ORDER BY a.DT, a.EmpNo
	
	IF @showSLU = 1 
		UPDATE @tempTable1 SET Flg = 'y' WHERE Txt = 'SLU' 
	ELSE 
		UPDATE @tempTable1 SET Flg = 'n' WHERE Txt = 'SLU'
	...
	IF @showILU = 1 
		UPDATE @tempTable1 SET Flg = 'y' WHERE Txt = 'ILU' 
	ELSE 
		UPDATE @tempTable1 SET Flg = 'n' WHERE Txt = 'ILU'
	...
	IF @showAL = 1 
		UPDATE @tempTable1 SET Flg = 'y' WHERE Txt = 'AL.' 
	ELSE 
		UPDATE @tempTable1 SET Flg = 'n' WHERE Txt = 'AL.'

	IF @showIL = 1 
		UPDATE @tempTable1 SET Flg = 'y' WHERE Txt = 'IL.' 
	ELSE 
		UPDATE @tempTable1 SET Flg = 'n' WHERE Txt = 'IL.'
	...
	UPDATE @tempTable2 set Txt = replace(Txt, 'SLU', 'SL|')
	UPDATE @tempTable2 set Txt = replace(Txt, 'ILU', 'IL|')
	UPDATE @tempTable2 set Txt = replace(Txt, 'SLP', 'SP|')
	UPDATE @tempTable2 set Txt = replace(Txt, 'AL.', 'AL|')
	UPDATE @tempTable2 set Txt = replace(Txt, 'IL.', 'IL|')
	...
	IF @showSLU = 1 UPDATE @tempTable2 SET Txt = REPLACE(Txt , 'SL', '<b>SL</b>')
	IF @showUL = 1 UPDATE @tempTable2 SET Txt = REPLACE(Txt , 'UL', '<b>UL</b>')
	IF @showILU = 1 UPDATE @tempTable2 SET Txt = REPLACE(Txt , 'IL' + @ch1 , '<b>IL' + @ch1 + '</b>' )
	IF @showSLP = 1 UPDATE @tempTable2 SET Txt = REPLACE(Txt , 'SP', '<b>SP</b>')
	IF @showAL = 1 UPDATE @tempTable2 SET Txt = REPLACE(Txt , 'AL', '<b>AL</b>')

	SELECT a.ActualCostCenter, a.EmpNo, a.EmpName, a.TXT AS AttendanceHistory,
		b.LeaveStartDate, b.LeaveEndDate, 
		c.IsDayOffBeforeStartDate, c.IsDayOffAfterEndDate, c.DayOffArray, c.LeaveType, b.ShiftPatCode
	FROM @tempTable2 a
		CROSS APPLY
		(
			SELECT EmpNo, MIN(DT) AS LeaveStartDate, MAX(DT) AS LeaveEndDate, ShiftPatCode
			FROM tas.Tran_Timesheet WITH (NOLOCK)
			WHERE RTRIM(LeaveType) IN ('UL', 'SLU', 'ILU', 'SLP', 'IL', 'AL')
				AND DT BETWEEN @startDate AND @endDate
				AND IsLastRow = 1
				AND EmpNo = a.EmpNo
			GROUP BY EmpNo, ShiftPatCode
		) b
		CROSS APPLY
		(
			SELECT * FROM tas.fnGetDayOffToFlagLeave(a.EmpNo, b.LeaveStartDate, b.LeaveEndDate)
		) c
		LEFT JOIN tas.Master_ShiftPatternTitles d WITH (NOLOCK) ON RTRIM(b.ShiftPatCode) = RTRIM(d.ShiftPatCode) AND d.IsDayShift = 1
	WHERE 
		(	
			--(Note: This flag controls the number of occurence)	
			(Flg LIKE '%yyyyy%' AND d.AutoID IS NOT NULL)	--(Note: Minimum of 5 days occurence Day-shift workers
			OR (Flg LIKE '%yyy%' AND d.AutoID IS NULL)		--(Note: Minimum of 3 days occurence Shift workers
		)
		AND ISNULL(c.DayOffArray, '') <> ''
	ORDER BY c.LeaveType, ActualCostCenter, EmpNo
...
PARAMETERS:
	...
	--Live database
	EXEC tas.Pr_GetEmpWithLongUnpaidLeave '04/16/2019', '05/15/2019', 0, '' 

	--Test database
	EXEC tas.Pr_GetEmpWithLongUnpaidLeave '11/16/2015', '12/15/2015', 0, ''		--Get all unpaid injury leave
	EXEC tas.Pr_GetEmpWithLongUnpaidLeave '02/16/2016', '03/15/2016', 0, ''		--Get all unpaid sick leave
	EXEC tas.Pr_GetEmpWithLongUnpaidLeave '02/16/2016', '03/15/2016', 0, ''		--Get all unpaid leave
