...
Stored Procedure Name	:	SecUser.pr_GetLeaveBalance
Description				:	This stored procedure retrieves the leave balance of the employee
...
Parameters
	@empNo				:	The employee no. from whom the leave balance will be retrieved
	@leaveType			:	The type of leave to retrieve
	@inquireDate		:	The leave balance as of the date
	@leaveBalance		:	The total leave balance based on the date

Revision History:
	1.0					NGF				2009.01.29 16:53
	Created

	1.1					SAK				2014.12.31 14:53
	Modified	

	1.2					EOB				2015.02.12 15:00
	Implemented the half day leave. Belows summarizes the changes applied:
	- Commented the rounding and removal of decimal digits value of @leaveBalance variable
	- Round the total leave balance to the nearest 0.5

	1.3					EOB				2015.06.18 09:11
	Added condition that sets the value of @inquireDate parameter to current date if the employee's company code is 00850

	1.4					EOB				2015.06.25 10:35
	Refactored the logic in calculating the total leave balance. Use the variable @leaveDecimal to store leave balance remainder

	1.5					EOB				2016.08.25 11:05
	Added condition to round leave balance to the nearest thousandths if decimal value is greater then 0.750

	1.6					EOB				2017.02.26 09:36
	Commented condition that checks if @leaveDecimal > 0.750

***********************************************************************************************************************************************************************/

ALTER PROCEDURE tas.pr_GetLeaveBalance
@empNo int,
@leaveType varchar(5),
@inquireDate smalldatetime,
@leaveBalance float output
AS
	...
	************************************************************************************************/
	...
	BEGIN
		...
		SELECT a.XXY58VCAFG, a.XXRMK, a.XXY58VCVDR / 10000
			FROM tas.sy_F55LVINQ AS a
				INNER JOIN tas.sy_LeaveRequisition AS b ON a.XXY58VCRQN = b.RequisitionNo
			WHERE a.XXAN8 = @empNo AND a.XXY58VCVCD = @leaveType 
				AND tas.ConvertFromJulian(a.XXY58VCOFD) > GETDATE()
				AND b.RequestStatusSpecialHandlingCode = 'Open'
	...
	************************************************************************************************/
	...
	--ELSE IF @leaveDecimal > 0.750	--Rev. #1.6
	--	SET @leaveBalance = FLOOR(@leaveBalance + 0.500)
	ELSE
		SET @leaveBalance = FLOOR(@leaveBalance) + 0.500
	/****************************** End of Rev. #1.4 **********************************************/

	--Commented the rounding and removal of decimal digits for the leave balance (Revision #1.2)
	-- Round-off the leave balance by removing the decimals
	--SELECT @leaveBalance = ROUND(@leaveBalance, 1)
	--SELECT @leaveBalance = FLOOR(@leaveBalance)

	--PRINT 'Leave Balance Start Date / UOM: ' + CONVERT(varchar(20), @leaveBalance)
...
/*	Debugging:
	...
	EXEC	@return_value = tas.pr_GetLeaveBalance
			@empNo = 10003632,
			@leaveType = N'AL',
			@inquireDate = '12/31/2017',
			@leaveBalance = @leaveBalance OUTPUT

	SELECT	@leaveBalance as N'@leaveBalance'
