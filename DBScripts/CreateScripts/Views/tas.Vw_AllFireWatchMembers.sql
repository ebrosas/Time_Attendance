...
*	Revision History
*
*	Name: tas.Vw_AllFireWatchMembers
*	Description: Get the list of all Fire Watch nenbers
*
...
ALTER VIEW tas.Vw_AllFireWatchMembers
AS		
	...
	FROM tas.sy_NAMES a WITH (NOLOCK)  
		...
		LEFT JOIN tas.sy_EvnLog c WITH (NOLOCK)  ON LTRIM(RTRIM(a.FName)) = LTRIM(RTRIM(c.FName)) 
			AND 
			(
			...
			AND c.[Event] = 8
		LEFT JOIN tas.Master_AccessReaders d WITH (NOLOCK)  ON c.Loc = d.LocationCode AND c.Dev = d.ReaderNo
	WHERE b.UdfNum = 14 
...
/* Testing:
	...
	SELECT * FROM tas.Vw_AllFireWatchMembers a
