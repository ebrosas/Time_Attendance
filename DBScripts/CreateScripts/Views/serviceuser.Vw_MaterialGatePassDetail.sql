...
/*	Debugging:
	...
	--Test server
	SELECT a.Quantity, * FROM serviceuser.Vw_MaterialGatePassDetail a		--Washcast scrap gate pass
	WHERE a.ServiceRequestNo IN (14788, 14787, 14786)

	--Production server
	SELECT a.Quantity, * FROM serviceuser.Vw_MaterialGatePassDetail a		--Washcast scrap gate pass
	WHERE a.ServiceRequestNo IN (66960, 66959, 66957)

	--"Normal Waste" category
	SELECT * FROM serviceuser.Vw_MaterialGatePassDetail a		--Closed
	WHERE a.ServiceRequestNo = 11466

	SELECT * FROM serviceuser.Vw_MaterialGatePassDetail a		--Open (Note: Already approved by Authorized Gate Pass approver)
	WHERE a.ServiceRequestNo = 11493

	--"Waste for Selling" category
	SELECT * FROM serviceuser.Vw_MaterialGatePassDetail a		--Closed
	WHERE a.ServiceRequestNo = 11451

	SELECT * FROM serviceuser.Vw_MaterialGatePassDetail a		--Open (Note: Not yet approved by Purchasing)
	WHERE a.ServiceRequestNo = 11495
	...
	SELECT * FROM serviceuser.Vw_MaterialGatePassDetail a		--Open (Note: Not yet approved by Purchasing)
	WHERE a.ServiceRequestNo = 11494

	SELECT * FROM serviceuser.Vw_MaterialGatePassDetail a		--Open (Note: Already approved by Purchasing)
	WHERE a.ServiceRequestNo = 11492
