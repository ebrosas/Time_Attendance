...
*	22/09/2020		Ervin		1.6			Removed filter by DateResigned, and set Pay Status to zero if YAPAST = 'P'
*	03/06/2021		Ervin		1.7			Uncommented the filter clause that exclude resigned employees
************************************************************************************************************************************************/
...
AS
	...
	SELECT	CAST(a.YAAN8 AS INT) AS EmpNo, 
			...
			CASE WHEN ISNUMERIC(ISNULL(a.YAPGRD, '0')) = 1 
				...
				ELSE 0 
			END AS GradeCode,
			CASE WHEN (a.YAPAST IN ('R', 'T', 'E', 'X') AND GETDATE() < tas.ConvertFromJulian(a.YADT)  OR UPPER(LTRIM(RTRIM(a.YAPAST))) IN ('I', 'A', 'P')) THEN '0' ELSE a.YAPAST END AS PayStatus,		--Rev. #1.6
	...
	FROM tas.syJDE_F060116 a WITH (NOLOCK)
		...
		LEFT JOIN tas.Master_Employee_JDE o WITH (NOLOCK) ON CAST(a.YAAN8 AS INT) = o.EmpNo	
...
/*	Debug:
	...
	SELECT * FROM tas.Vw_EmployeeDirectory a
