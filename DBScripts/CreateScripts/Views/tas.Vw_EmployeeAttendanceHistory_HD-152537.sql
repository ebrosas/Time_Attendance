...
*	09/03/2021		Ervin		2.9			Refactored the code to implement Helpdesk Enhancement Request No. 152537 
*****************************************************************************************************************************************************************************************************/
...
AS
	...
	SELECT	a.AutoID,
			...
			--END AS ShiftCode,
			RTRIM(f.Effective_ShiftCode) AS ShiftCode,		--Rev. #2.9
			...
			CASE WHEN a.dtIN IS NOT NULL AND a.dtOUT IS NOT NULL AND (ISNULL(a.LeaveType, '') = '' OR RTRIM(a.LeaveType) = 'HD')
			...
			CASE WHEN a.Shaved_IN IS NOT NULL AND a.Shaved_OUT IS NOT NULL AND (ISNULL(a.LeaveType, '') = '' OR RTRIM(a.LeaveType) = 'HD')
			...
			--CASE WHEN RTRIM(f.Effective_ShiftCode) = 'O' AND ISNULL(a.LeaveType, '' ) <> '' THEN 0
			--	ELSE tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), RTRIM(f.Effective_ShiftCode))
			--END AS Duration_Required,		--Rev. #2.9
			tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), RTRIM(f.Effective_ShiftCode)) AS Duration_Required,		--Rev. #2.9

			--CASE WHEN RTRIM(a.ShiftCode) = 'O' AND (a.dtIN IS NULL AND a.dtOUT IS NULL) 
			CASE WHEN RTRIM(f.Effective_ShiftCode) = 'O' AND a.Duration_Worked_Cumulative = 0		--Rev. #2.9
				THEN 
					CASE WHEN ISNULL(a.IsDayWorker_OR_Shifter, 0) = 0 
						THEN tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), 'M')	--Shift Worker
						ELSE tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), 'D')	--Day Shift Worker
					END 
				ELSE 0
			...
			CASE WHEN ISNULL(a.LeaveType, '') <> '' AND RTRIM(a.LeaveType) <> 'HD'
				THEN 
					CASE WHEN ISNULL(a.IsDayWorker_OR_Shifter, 0) = 0 
						THEN tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), 'M')	--Shift Worker
						ELSE tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), 'D')	--Day Shift Worker
			...
			CASE WHEN RTRIM(a.RemarkCode) = 'A'
				THEN 
					CASE WHEN ISNULL(a.IsDayWorker_OR_Shifter, 0) = 0 
						THEN tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), 'M')	--Shift Worker
						ELSE tas.fnGetRequiredWorkDuration(RTRIM(a.ShiftPatCode), 'D')	--Day Shift Worker
					END 
				ELSE 0
			END AS AbsentDuration				
	FROM tas.Tran_Timesheet	a WITH (NOLOCK)
		LEFT JOIN tas.GetRemark02 b WITH (NOLOCK) ON a.AutoId = b.AutoId
		INNER JOIN tas.Master_ShiftPatternTitles c WITH (NOLOCK) ON RTRIM(a.ShiftPatCode) = RTRIM(c.ShiftPatCode) 	
		...
		INNER JOIN tas.Tran_ShiftPatternUpdates f WITH (NOLOCK) ON a.EmpNo = f.EmpNo AND a.DT = f.DateX	--Rev. #2.2
...
/*	Debug:
	...
	SELECT * FROM tas.Vw_EmployeeAttendanceHistory a
	WHERE a.EmpNo = 10003631
		AND a.DT BETWEEN '09/16/2020' AND '10/15/2020'
	ORDER BY a.DT DESC
