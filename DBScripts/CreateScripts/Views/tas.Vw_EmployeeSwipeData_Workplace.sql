...
*	13/09/2022		Ervin		2.6			Disabled checking if IsDayshift = 0
****************************************************************************************************************************************************************/
...
AS
	...
	(
		...
		(
			...
			WHERE a.EventCode = 8	--(Note: 8 means successful swipe)
				--AND ISNULL(d.IsDayShift, 0) = 0	--Rev. #2.6
...
/*	Debug:
	...
	WHERE a.SwipeDate BETWEEN '04/10/2022' AND '04/11/2022'
		AND RTRIM(a.BusinessUnit) = '7600'
	ORDER BY BusinessUnit, EmpNo
