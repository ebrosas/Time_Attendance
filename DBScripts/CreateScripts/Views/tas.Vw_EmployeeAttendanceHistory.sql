...
*	22/08/2022		Ervin		3.4			Commented filter that checks if IsSyncTimesheet = 1
*	10/11/2022		Ervin		3.5			Bypass all other attendance remarks if employee was absent
*	14/12/2022		Ervin		3.6			Refactored the code in fetching the TimeInWP and TimeOutWP
*	13/04/2023		Ervin		3.7			Enabled fetching data for year 2021
*	23/10/2023		Ervin		3.8			Fixed bug reported by HR with regards to wrong value of TimeInMG and TimeOutMG during dayoff
*****************************************************************************************************************************************************************************************************/
...
AS
	...
	SELECT	a.AutoID,
			...
			CASE WHEN a.RemarkCode = 'A' THEN 'Absent' ELSE 
			(
				...
				(CASE WHEN RTRIM(ISNULL(a.LeaveType, '')) = 'DD' OR RTRIM(ISNULL(a.AbsenceReasonCode, '')) = 'DD' THEN 'Day In Lieu' ELSE '' END)
			) END AS Remarks,
				
			CASE WHEN k.IsWorkplaceEnabled = 1 /* AND k.IsSyncTimesheet = 1 */ AND a.DT >= l.EffectiveDate THEN 1 ELSE 0 END AS RequiredToSwipeAtWorkplace,		--Rev. #3.4
				
			CASE WHEN ISNULL(e.timeIN, '') <> '' AND RTRIM(ISNULL(a.ShiftCode, l.EffectiveDate)) <> 'O'		--Rev. #3.8
			...
			CASE WHEN d.CorrectionType IN (1, 3)	--(Note: 1 = Workplace Time In; 3 = Both)
				...
				ELSE
					CASE WHEN RTRIM(f.Effective_ShiftCode) = 'O' 
						...
						ELSE --i.SwipeTime 
							CASE WHEN i.SwipeTime = j.SwipeTime THEN d.TimeInWP ELSE i.SwipeTime END	--Rev. #3.6
					END 
			END AS TimeInWP,
			CASE WHEN d.CorrectionType IN (2, 3)	--(Note: 2 = Workplace Time Out; 3 = Both)
				...
				ELSE
					CASE WHEN RTRIM(f.Effective_ShiftCode) = 'O' 
						...
						ELSE --j.SwipeTime 
							CASE WHEN i.SwipeTime = j.SwipeTime THEN d.TimeOutWP ELSE j.SwipeTime END		--Rev. #3.6
					END 
			END AS TimeOutWP,
			CASE WHEN ISNULL(e.[timeOUT], '') <> '' AND RTRIM(ISNULL(a.ShiftCode, l.EffectiveDate)) <> 'O'		--Rev. #3.8
	...
	/*	Note: Uncomment this block of code to fetch attendance records from the Timesheet archive table for year 2021	(Rev. #3.7)
	UNION
	...
	FROM [Archive].[dbo].[tas2_Tran_Timesheet_2021] a WITH (NOLOCK)
		LEFT JOIN tas.GetRemark02 b WITH (NOLOCK) ON a.AutoId = b.AutoId
		LEFT JOIN tas.Master_ShiftPatternTitles c WITH (NOLOCK) ON RTRIM(a.ShiftPatCode) = RTRIM(c.ShiftPatCode) 	
		LEFT JOIN tas.Tran_WorkplaceSwipe d WITH (NOLOCK) ON a.EmpNo = d.EmpNo AND a.DT = d.SwipeDate
	*/
