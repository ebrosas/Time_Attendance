...
/****** Object:  View [tas].[Vw_ContractorAttendance_V2]    Script Date: 11/11/2021 15:51:28 ******/
SET ANSI_NULLS ON
GO
...
/*	Debugging:
	...
	WHERE a.EmpNo IN (61024)

	SELECT * FROM tas.Vw_ContractorAttendance_V2 a
	WHERE a.RequiredWorkDuration > 0

	SELECT * FROM tas.Vw_ContractorAttendance_V2 a
	WHERE UPPER(RTRIM(a.EmployerName)) LIKE '%MI CONTRACT%'
		AND A.StatusID = 1

	SELECT * FROM tas.Vw_ContractorAttendance_V2 a
	WHERE UPPER(RTRIM(a.EmployerName)) LIKE '%M I%'
		AND A.StatusID = 1
