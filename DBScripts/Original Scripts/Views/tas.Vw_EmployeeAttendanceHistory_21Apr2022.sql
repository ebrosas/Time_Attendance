...
/****** Object:  View [tas].[Vw_EmployeeAttendanceHistory]    Script Date: 21/04/2022 15:05:51 ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	SELECT	a.AutoID,
			...
			--	THEN
			--		CASE WHEN d.IsCorrected = 1 AND d.IsClosed = 1 AND RTRIM(ISNULL(d.StatusHandlingCode, '')) NOT IN ('Cancelled', 'Rejected') 
			--			THEN CASE WHEN RTRIM(f.Effective_ShiftCode) IN ('O', 'N') THEN d.TimeInWP ELSE i.SwipeDateTime END		--Rev. #3.1
			--			ELSE NULL 
			--		END 
			--	ELSE
			--		CASE WHEN RTRIM(f.Effective_ShiftCode) IN ('O', 'N') THEN d.TimeInWP ELSE i.SwipeDateTime END	--Rev. #3.1
			--END AS TimeInWP,		
				
			--CASE WHEN d.CorrectionType IN (2, 3)	--(Note: 2 = Workplace Time Out; 3 = Both)
			--	THEN
			--		CASE WHEN d.IsCorrected = 1 AND d.IsClosed = 1 AND RTRIM(ISNULL(d.StatusHandlingCode, '')) NOT IN ('Cancelled', 'Rejected') 
			--			THEN CASE WHEN RTRIM(f.Effective_ShiftCode) IN ('O', 'N') THEN d.TimeOutWP ELSE j.SwipeDateTime END		--Rev. #3.1
			--			ELSE NULL 
			--		END 
			--	ELSE
			--		CASE WHEN RTRIM(f.Effective_ShiftCode) IN ('O', 'N') THEN d.TimeOutWP ELSE j.SwipeDateTime END	--Rev. #3.1
			--END AS TimeOutWP,
			--End of Rev. #3.0
			...
			CASE WHEN d.CorrectionType IN (1, 3)	--(Note: 1 = Workplace Time In; 3 = Both)
				THEN
					CASE WHEN d.IsCorrected = 1 /*AND d.IsClosed = 1*/ AND RTRIM(ISNULL(d.StatusHandlingCode, '')) NOT IN ('Cancelled', 'Rejected') THEN d.TimeInWP ELSE i.SwipeDateTime END 
				ELSE
					CASE WHEN RTRIM(f.Effective_ShiftCode) = 'O' 
						THEN CASE WHEN ISNULL(a.Actual_ShiftCode, '') NOT IN ('', 'N') --THEN i.SwipeDateTime ELSE d.TimeInWP END 
							THEN CASE WHEN i.SwipeDateTime = j.SwipeDateTime THEN a.dtIn ELSE i.SwipeDateTime END 
							ELSE d.TimeOutWP 
						END 
						WHEN RTRIM(f.Effective_ShiftCode) = 'N' OR RTRIM(a.Actual_ShiftCode) = 'N' THEN d.TimeInWP 
						ELSE i.SwipeDateTime 
					END 
			END AS TimeInWP,
			CASE WHEN d.CorrectionType IN (2, 3)	--(Note: 2 = Workplace Time Out; 3 = Both)
				THEN
					CASE WHEN d.IsCorrected = 1 /*AND d.IsClosed = 1*/ AND RTRIM(ISNULL(d.StatusHandlingCode, '')) NOT IN ('Cancelled', 'Rejected') THEN d.TimeOutWP ELSE j.SwipeDateTime  END 
				ELSE
					CASE WHEN RTRIM(f.Effective_ShiftCode) = 'O' 
						THEN CASE WHEN ISNULL(a.Actual_ShiftCode, '')  NOT IN ('', 'N') 
							THEN CASE WHEN j.SwipeDateTime = i.SwipeDateTime THEN a.dtOut ELSE j.SwipeDateTime END 
							ELSE d.TimeOutWP 
						END 
						WHEN RTRIM(f.Effective_ShiftCode) = 'N' OR RTRIM(a.Actual_ShiftCode) = 'N' THEN d.TimeOutWP 
						ELSE j.SwipeDateTime 
					END 
			END AS TimeOutWP,
			/***************************************************** End of Rev. #3.2 *********************************************************************************/
	...
	FROM tas.Tran_Timesheet	a WITH (NOLOCK)
		...
		OUTER APPLY 
		(
			--SELECT TOP 1 SwipeDateTime FROM tas.Vw_WorkplaceReaderSwipe WITH (NOLOCK)
			--WHERE EmpNo = a.EmpNo
			--	AND SwipeDate = a.DT
			--ORDER BY SwipeTime ASC

			--Start of Rev. #3.2
			SELECT TOP 1 x.SwipeDateTime 
