...
/****** Object:  View [tas].[Vw_EmployeeAttendanceHistory]    Script Date: 23/02/2022 13:40:32 ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	SELECT	a.AutoID,
			...
			CASE WHEN d.CorrectionType IN (1, 3)	--(Note: 1 = Workplace Time In; 3 = Both)
				THEN
					CASE WHEN d.IsCorrected = 1 AND d.IsClosed = 1 AND RTRIM(ISNULL(d.StatusHandlingCode, '')) NOT IN ('Cancelled', 'Rejected') THEN i.SwipeDateTime ELSE NULL END 
				ELSE
					i.SwipeDateTime
			END AS TimeInWP,			
			CASE WHEN d.CorrectionType IN (2, 3)	--(Note: 2 = Workplace Time Out; 3 = Both)
				THEN
					CASE WHEN d.IsCorrected = 1 AND d.IsClosed = 1 AND RTRIM(ISNULL(d.StatusHandlingCode, '')) NOT IN ('Cancelled', 'Rejected') THEN j.SwipeDateTime ELSE NULL END 
				ELSE
					j.SwipeDateTime
			END AS TimeOutWP,
			--End of Rev. #3.0
