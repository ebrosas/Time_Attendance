...
/****** Object:  View [tas].[Vw_EmployeeDirectory]    Script Date: 15/08/2018 10:31:47 ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	FROM tas.syJDE_F060116 a
		...
		LEFT JOIN tas.syJDE_F00092 c ON a.YAAN8 = c.T3SBN1 AND LTRIM(RTRIM(c.T3TYDT)) = 'WH' AND LTRIM(RTRIM(c.T3SDB)) = 'E'
		--LEFT JOIN tas.syJDE_F0101 d ON a.YAAN8 = d.ABAN8
		LEFT JOIN tas.syJDE_F0115 e ON a.YAAN8 = e.WPAN8 AND LTRIM(RTRIM(e.WPPHTP)) = 'EXT' 
		--LEFT JOIN tas.syJDE_F0115 f ON a.YAAN8 = f.WPAN8 AND LTRIM(RTRIM(f.WPPHTP)) = 'MOBS' 		
		LEFT JOIN tas.syJDE_F0115 g ON a.YAAN8 = g.WPAN8 AND LTRIM(RTRIM(g.WPPHTP)) = 'DL' 		
		LEFT JOIN tas.syJDE_F0115 h ON a.YAAN8 = h.WPAN8 AND LTRIM(RTRIM(h.WPPHTP)) = 'F' 		
		LEFT JOIN tas.syjde_F01151 i ON a.YAAN8 = i.EAAN8 AND i.EAIDLN = 0 AND i.EARCK7 = 1 AND UPPER(LTRIM(RTRIM(i.EAETP))) = 'E' 
		LEFT JOIN tas.syJDE_F0101 j ON a.YAAN8 = j.ABAN8
		LEFT JOIN tas.Master_EmployeeAdditional k ON CAST(a.YAAN8 AS INT) = k.EmpNo		
		LEFT JOIN tas.syJDE_F0005 l ON LTRIM(RTRIM(a.YAEEOM)) = LTRIM(RTRIM(l.DRKY)) AND RTRIM(LTRIM(l.DRSY)) = '06' AND RTRIM(LTRIM(l.DRRT)) = 'M'
		LEFT JOIN tas.syJDE_F0005 m ON LTRIM(RTRIM(a.YASEX)) = LTRIM(RTRIM(m.DRKY)) AND RTRIM(LTRIM(m.DRSY)) = '01' AND RTRIM(LTRIM(m.DRRT)) = '29'
		LEFT JOIN tas.syJDE_F0005 n ON LTRIM(RTRIM(a.YAEEOJ)) = LTRIM(RTRIM(n.DRKY)) AND RTRIM(LTRIM(n.DRSY)) = '06' AND RTRIM(LTRIM(n.DRRT)) = 'J'
		INNER JOIN tas.Master_Employee_JDE o ON CAST(a.YAAN8 AS INT) = o.EmpNo	
