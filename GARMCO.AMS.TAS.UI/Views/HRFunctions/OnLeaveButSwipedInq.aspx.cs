...
{
    public partial class OnLeaveButSwipedInq : BaseWebForm, IFormExtension
    {
        #region Enumeration
        ...
        private Dictionary<string, object> OnLeaveButSwipedStorage
        {
            get
            {
                Dictionary<string, object> list = Session["OnLeaveButSwipedStorage"] as Dictionary<string, object>;
                if (list == null)
                    Session["OnLeaveButSwipedStorage"] = list = new Dictionary<string, object>();

                return list;
            }
            set
            {
                Session["OnLeaveButSwipedStorage"] = value;
            }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                this.Master.SetPageForm(UIHelper.FormAccessCodes.LEAVESWIPE.ToString());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                this.Master.FormTitle = UIHelper.PAGE_ONLEAVE_BUTSWIPED_INQUIRY_TITLE;
                #endregion
                ...
                if (!GAPFunction.CheckFormAccess(this.Master.FormAccess, GAPConstants.FormAccessIndex.Retrieve))
                {
                    ...
                    {
                        Response.Redirect(String.Format("{0}?error={1}&pageName={2}", UIHelper.PAGE_ERROR, Convert.ToInt32(UIHelper.PageErrorCodes.NoAccessOnPage), UIHelper.PAGE_ONLEAVE_BUTSWIPED_INQUIRY_TITLE), true);
                    }
                }
                #endregion
                ...
                if (this.OnLeaveButSwipedStorage.Count > 0)
                {
                    if (this.OnLeaveButSwipedStorage.ContainsKey("FormFlag"))
                        formFlag = UIHelper.ConvertObjectToString(this.OnLeaveButSwipedStorage["FormFlag"]);
                }
                #endregion
                ...
                if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.GetEmployeeInfo.ToString())
                {
                    ...
                    if (UIHelper.ConvertObjectToInt(Session[UIHelper.CONST_EMPLOYEE_SEARCH_FLAG]) == 1)
                    {
                        ...
                        //this.cboCostCenter.Text = UIHelper.ConvertObjectToString(Request.QueryString[UIHelper.QUERY_STRING_COSTCENTER_KEY]);
                    ...
                    // Clear data storage
                    Session.Remove("OnLeaveButSwipedStorage");
                    #endregion
                }
        ...
        protected void btnFindEmployee_Click(object sender, EventArgs e)
        {
            ...
            (
                ...
                UIHelper.PAGE_ONLEAVE_BUTSWIPED_INQUIRY
            ),
            false);
        }
        #endregion
        ...
        private void RestoreDataFromCollection()
        {
            ...
            if (this.OnLeaveButSwipedStorage.ContainsKey("CallerForm"))
                this.CallerForm = UIHelper.ConvertObjectToString(this.OnLeaveButSwipedStorage["CallerForm"]);
            else
                this.CallerForm = string.Empty;
            #endregion

            #region Restore session values
            if (this.OnLeaveButSwipedStorage.ContainsKey("AttendanceList"))
                this.AttendanceList = this.OnLeaveButSwipedStorage["AttendanceList"] as List<EmployeeAttendanceEntity>;
            else
                this.AttendanceList = null;

            if (this.OnLeaveButSwipedStorage.ContainsKey("EmployeeNo"))
                this.EmployeeNo = UIHelper.ConvertObjectToInt(this.OnLeaveButSwipedStorage["EmployeeNo"]);
            else
                this.EmployeeNo = 0;

            if (this.OnLeaveButSwipedStorage.ContainsKey("CurrentStartRowIndex"))
                this.CurrentStartRowIndex = UIHelper.ConvertObjectToInt(this.OnLeaveButSwipedStorage["CurrentStartRowIndex"]);
            else
                this.CurrentStartRowIndex = 0;

            if (this.OnLeaveButSwipedStorage.ContainsKey("CurrentMaximumRows"))
                this.CurrentMaximumRows = UIHelper.ConvertObjectToInt(this.OnLeaveButSwipedStorage["CurrentMaximumRows"]);
            else
                this.CurrentMaximumRows = 0;

            if (this.OnLeaveButSwipedStorage.ContainsKey("CurrentPageIndex"))
                this.CurrentPageIndex = UIHelper.ConvertObjectToInt(this.OnLeaveButSwipedStorage["CurrentPageIndex"]);
            else
                this.CurrentPageIndex = 0;

            if (this.OnLeaveButSwipedStorage.ContainsKey("CurrentPageSize"))
                this.CurrentPageSize = UIHelper.ConvertObjectToInt(this.OnLeaveButSwipedStorage["CurrentPageSize"]);
            else
            ...
            if (this.OnLeaveButSwipedStorage.ContainsKey("txtEmpNo"))
                this.txtEmpNo.Text = UIHelper.ConvertObjectToString(this.OnLeaveButSwipedStorage["txtEmpNo"]);
            else
                this.txtEmpNo.Text = string.Empty;

            if (this.OnLeaveButSwipedStorage.ContainsKey("litEmpName"))
                this.litEmpName.Text = UIHelper.ConvertObjectToString(this.OnLeaveButSwipedStorage["litEmpName"]);
            else
                this.litEmpName.Text = string.Empty;

            if (this.OnLeaveButSwipedStorage.ContainsKey("cboCostCenter"))
                this.cboCostCenter.SelectedValue = UIHelper.ConvertObjectToString(this.OnLeaveButSwipedStorage["cboCostCenter"]);
            else
            {
                this.cboCostCenter.Text = string.Empty;
                this.cboCostCenter.SelectedIndex = -1;
            ...
            if (this.OnLeaveButSwipedStorage.ContainsKey("dtpStartDate"))
                this.dtpStartDate.SelectedDate = UIHelper.ConvertObjectToDate(this.OnLeaveButSwipedStorage["dtpStartDate"]);
            else
                this.dtpStartDate.SelectedDate = null;

            if (this.OnLeaveButSwipedStorage.ContainsKey("dtpEndDate"))
                this.dtpEndDate.SelectedDate = UIHelper.ConvertObjectToDate(this.OnLeaveButSwipedStorage["dtpEndDate"]);
            else
                this.dtpEndDate.SelectedDate = null;
            #endregion
        ...
        {
            this.OnLeaveButSwipedStorage.Clear();
            this.OnLeaveButSwipedStorage.Add("FormFlag", formFlag.ToString());

            #region Save control values to session
            this.OnLeaveButSwipedStorage.Add("txtEmpNo", this.txtEmpNo.Text.Trim());
            this.OnLeaveButSwipedStorage.Add("litEmpName", this.litEmpName.Text.Trim());
            this.OnLeaveButSwipedStorage.Add("cboCostCenter", this.cboCostCenter.SelectedValue);
            this.OnLeaveButSwipedStorage.Add("dtpStartDate", this.dtpStartDate.SelectedDate);
            this.OnLeaveButSwipedStorage.Add("dtpEndDate", this.dtpEndDate.SelectedDate);
            #endregion

            #region Save Query String values to collection
            this.OnLeaveButSwipedStorage.Add("CallerForm", this.CallerForm);
            #endregion

            #region Store session data to collection
            this.OnLeaveButSwipedStorage.Add("AttendanceList", this.AttendanceList);
            this.OnLeaveButSwipedStorage.Add("EmployeeNo", this.EmployeeNo);
            this.OnLeaveButSwipedStorage.Add("CurrentStartRowIndex", this.CurrentStartRowIndex);
            this.OnLeaveButSwipedStorage.Add("CurrentMaximumRows", this.CurrentMaximumRows);
            this.OnLeaveButSwipedStorage.Add("CurrentPageIndex", this.CurrentPageIndex);
            this.OnLeaveButSwipedStorage.Add("CurrentPageSize", this.CurrentPageSize);
            #endregion
        }
        ...
        private void GetOnAnnualLeaveBuSwiped(bool reloadDataFromDB = false)
        {
            try
            {
                ...
                else
                {
                    ...
                    DALProxy proxy = new DALProxy();
                    var source = proxy.GetOnAnnualLeaveBuSwiped(startDate, endDate, empNo, costCenter, this.CurrentPageIndex, this.CurrentPageSize, ref error, ref innerError);
        ...
        private void GetOnAnnualLeaveBuSwipedOld()
        {
            //try
            //{
            //    #region Initialize variables               
            ...
            //    var rawData = WCFProxy.GetOnAnnualLeaveBuSwiped(startDate, endDate, empNo, costCenter, ref error, ref innerError);
            //    if (!string.IsNullOrEmpty(error) || !string.IsNullOrEmpty(innerError))
            //    {
            //        if (!string.IsNullOrEmpty(innerError))
            //            throw new Exception(error, new Exception(innerError));
