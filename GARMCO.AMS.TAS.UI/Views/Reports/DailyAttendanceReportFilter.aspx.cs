...
{
    public partial class DailyAttendanceReportFilter : BaseWebForm, IFormExtension
    {
        #region Enumeration
        ...
        private Dictionary<string, object> DailyAttendanceStorage
        {
            get
            {
                Dictionary<string, object> list = Session["DailyAttendanceStorage"] as Dictionary<string, object>;
                if (list == null)
                    Session["DailyAttendanceStorage"] = list = new Dictionary<string, object>();

                return list;
            }
            set
            {
                Session["DailyAttendanceStorage"] = value;
            }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                this.Master.SetPageForm(UIHelper.FormAccessCodes.DAILYREPRT.ToString());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                this.Master.FormTitle = UIHelper.PAGE_DAILY_ATTENDANCE_REPORT_TITLE;
                #endregion
                ...
                if (!GAPFunction.CheckFormAccess(this.Master.FormAccess, GAPConstants.FormAccessIndex.Retrieve))
                {
                    ...
                    {
                        Response.Redirect(String.Format("{0}?error={1}&pageName={2}", UIHelper.PAGE_ERROR, Convert.ToInt32(UIHelper.PageErrorCodes.NoAccessOnPage), UIHelper.PAGE_DAILY_ATTENDANCE_REPORT_TITLE), true);
                    }
                }
                #endregion
                ...
                if (this.DailyAttendanceStorage.Count > 0)
                {
                    if (this.DailyAttendanceStorage.ContainsKey("FormFlag"))
                        formFlag = UIHelper.ConvertObjectToString(this.DailyAttendanceStorage["FormFlag"]);
                }
                #endregion
                ...
                if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.ShowReport.ToString())
                {
                    ...
                    // Clear data storage
                    this.DailyAttendanceStorage.Clear();
                    #endregion
                }
                else
                {
        ...
        protected void btnShowReport_Click(object sender, EventArgs e)
        {
            ...
            // Check Employee Type
            if (string.IsNullOrEmpty(this.rblEmployeeType.SelectedValue))
            {
                this.txtGeneric.Text = ValidationErrorType.NoEmployeeType.ToString();
                this.ErrorType = ValidationErrorType.NoEmployeeType;
            ...
            GetDailyAttendanceReportDate(true);
                ...
                this.ReportDataList.Count > 0)
            {
                ...
                {
                    #region Display the Daily Attendance Report for Non-Salary Staff  
                    // Save report data to session
                    Session["DailyAttendanceNonSalarySource"] = this.ReportDataList;
                    ...
                    (
                        ...
                        UIHelper.ReportTypes.DailyAttendanceReportNonSalaryStaff.ToString(),
                        UIHelper.QUERY_STRING_CALLER_FORM_KEY,
                        UIHelper.PAGE_DAILY_ATTENDANCE_REPORT
                    ),
                    false);
                    #endregion
                }
                else if (this.rblEmployeeType.SelectedValue == EmployeeType.valSalaryStaff.ToString())
                {
                    ...
                    (
                        String.Format(UIHelper.PAGE_REPORT_VIEWER + "?{0}={1}&{2}={3}",
                        UIHelper.QUERY_STRING_REPORT_TYPE_KEY,
                        UIHelper.ReportTypes.DailyAttendanceReportSalaryStaff.ToString(),
                        UIHelper.QUERY_STRING_CALLER_FORM_KEY,
                        UIHelper.PAGE_DAILY_ATTENDANCE_REPORT
                    ),
                    false);
                ...
                {
                    #region Display the Daily Attendance Report for both Salary Staff and Non-Salary Staff
                    // Extract non-salary staff from the collection 
                    Session["DailyAttendanceNonSalarySource"] = this.ReportDataList.Where(a => a.IsSalaryStaff == false).ToList();

                    // Extract salary staff from the collection 
                    Session["DailyAttendanceSalarySource"] = this.ReportDataList.Where(a => a.IsSalaryStaff == true).ToList();
                    ...
                    (
                        String.Format(UIHelper.PAGE_REPORT_VIEWER + "?{0}={1}&{2}={3}",
                        UIHelper.QUERY_STRING_REPORT_TYPE_KEY,
                        UIHelper.ReportTypes.DailyAttendanceReportAll.ToString(),
                        UIHelper.QUERY_STRING_CALLER_FORM_KEY,
                        UIHelper.PAGE_DAILY_ATTENDANCE_REPORT
                    ),
                    false);
        ...
        private void RestoreDataFromCollection()
        {
            ...
            if (this.DailyAttendanceStorage.ContainsKey("CallerForm"))
                this.CallerForm = UIHelper.ConvertObjectToString(this.DailyAttendanceStorage["CallerForm"]);
            else
                this.CallerForm = string.Empty;

            if (this.DailyAttendanceStorage.ContainsKey("CurrentViewType"))
            {
                string viewType = UIHelper.ConvertObjectToString(this.DailyAttendanceStorage["CurrentViewType"]);
                if (viewType != string.Empty)
                {
                    ViewType filterOption = ViewType.NotDefined;
                    try
            ...
            if (this.DailyAttendanceStorage.ContainsKey("ReportDataList"))
                this.ReportDataList = this.DailyAttendanceStorage["ReportDataList"] as List<EmployeeAttendanceEntity>;
            else
                this.ReportDataList = null;

            if (this.DailyAttendanceStorage.ContainsKey("CostCenterList"))
                this.CostCenterList = this.DailyAttendanceStorage["CostCenterList"] as List<CostCenterEntity>;
            else
            ...
            if (this.DailyAttendanceStorage.ContainsKey("rblDateOption"))
                this.rblDateOption.SelectedValue = UIHelper.ConvertObjectToString(this.DailyAttendanceStorage["rblDateOption"]);
            else
                this.rblDateOption.SelectedValue = DateOption.valSpecificDate.ToString();

            this.rblDateOption_SelectedIndexChanged(this.rblDateOption, new EventArgs());

            if (this.DailyAttendanceStorage.ContainsKey("dtpStartDate"))
                this.dtpStartDate.SelectedDate = UIHelper.ConvertObjectToDate(this.DailyAttendanceStorage["dtpStartDate"]);
            else
                this.dtpStartDate.SelectedDate = null;

            if (this.DailyAttendanceStorage.ContainsKey("dtpEndDate"))
                this.dtpEndDate.SelectedDate = UIHelper.ConvertObjectToDate(this.DailyAttendanceStorage["dtpEndDate"]);
            else
                this.dtpEndDate.SelectedDate = null;

            if (this.DailyAttendanceStorage.ContainsKey("rblEmployeeType"))
                this.rblEmployeeType.SelectedValue = UIHelper.ConvertObjectToString(this.DailyAttendanceStorage["rblEmployeeType"]);
            else
                this.rblEmployeeType.SelectedValue = EmployeeType.valBoth.ToString();

            if (this.DailyAttendanceStorage.ContainsKey("lbCostCenter"))
            {
                if (this.lbCostCenter.Items.Count > 0)
                {
                    dynamic selectedItems = this.DailyAttendanceStorage["lbCostCenter"];
                    //List<RadListBoxItem> selectedItems = this.DailyAttendanceStorage["lbCostCenter"] as List<RadListBoxItem>;
            ...
            if (this.DailyAttendanceStorage.ContainsKey("chkSelectAll"))
                this.chkSelectAll.Checked = UIHelper.ConvertObjectToBolean(this.DailyAttendanceStorage["chkSelectAll"]);
            else
                this.chkSelectAll.Checked = false;
        ...
        {
            this.DailyAttendanceStorage.Clear();
            this.DailyAttendanceStorage.Add("FormFlag", formFlag.ToString());

            #region Save control values to session
            this.DailyAttendanceStorage.Add("rblDateOption", this.rblDateOption.SelectedValue);
            this.DailyAttendanceStorage.Add("dtpStartDate", this.dtpStartDate.SelectedDate);
            this.DailyAttendanceStorage.Add("dtpEndDate", this.dtpEndDate.SelectedDate);
            this.DailyAttendanceStorage.Add("rblEmployeeType", this.rblEmployeeType.SelectedValue);
            this.DailyAttendanceStorage.Add("lbCostCenter", this.lbCostCenter.CheckedItems);
            this.DailyAttendanceStorage.Add("chkSelectAll", this.chkSelectAll.Checked);
            #endregion

            #region Save Query String values to collection
            this.DailyAttendanceStorage.Add("CallerForm", this.CallerForm);
            this.DailyAttendanceStorage.Add("CurrentViewType", this.CurrentViewType);
            #endregion

            #region Store session data to collection
            this.DailyAttendanceStorage.Add("ReportDataList", this.ReportDataList);
            this.DailyAttendanceStorage.Add("CostCenterList", this.CostCenterList);
            #endregion
        }
        ...
        private void GetDailyAttendanceReportDate(bool reloadDataFromDB = false)
        {
            try
            {
                ...
                else
                {
                    ...
                    DALProxy proxy = new DALProxy();
                    var source = proxy.GetDailyAttendanceReportData(employeeType, startDate, endDate, costCenterList, ref error, ref innerError);
