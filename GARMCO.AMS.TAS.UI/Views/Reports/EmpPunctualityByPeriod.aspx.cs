...
{
    public partial class EmpPunctualityByPeriod : BaseWebForm, IFormExtension
    {
        #region Enumeration
        ...
        private Dictionary<string, object> UnunctualEmpStorage
        {
            get
            {
                Dictionary<string, object> list = Session["UnunctualEmpStorage"] as Dictionary<string, object>;
                if (list == null)
                    Session["UnunctualEmpStorage"] = list = new Dictionary<string, object>();

                return list;
            }
            set
            {
                Session["UnunctualEmpStorage"] = value;
            }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                this.Master.SetPageForm(UIHelper.FormAccessCodes.UNPUNCTUAL.ToString());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                this.Master.FormTitle = UIHelper.PAGE_UNPUNCTUAL_EMP_SUMMARY_REPORT_TITLE;
                #endregion
                ...
                if (!GAPFunction.CheckFormAccess(this.Master.FormAccess, GAPConstants.FormAccessIndex.Retrieve))
                {
                    ...
                    {
                        Response.Redirect(String.Format("{0}?error={1}&pageName={2}", UIHelper.PAGE_ERROR, Convert.ToInt32(UIHelper.PageErrorCodes.NoAccessOnPage), UIHelper.PAGE_UNPUNCTUAL_EMP_SUMMARY_REPORT_TITLE), true);
                    }
                }
                #endregion
                ...
                if (this.UnunctualEmpStorage.Count > 0)
                {
                    if (this.UnunctualEmpStorage.ContainsKey("FormFlag"))
                        formFlag = UIHelper.ConvertObjectToString(this.UnunctualEmpStorage["FormFlag"]);
                }
                #endregion
                ...
                if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.ShowReport.ToString())
                {
                    ...
                    // Clear data storage
                    this.UnunctualEmpStorage.Clear();
                    #endregion
                }
                else if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.GetEmployeeInfo.ToString())
                {
                    ...
                    // Clear data storage
                    Session.Remove("UnunctualEmpStorage");
                    #endregion
                }
                else
                {
        ...
        protected void btnShowReport_Click(object sender, EventArgs e)
        {
                ...
                this.ReportDataList.Count > 0)
            {
                ...
                Session["UnpunctualEmployeeReportSource"] = this.ReportDataList;
                ...
                (
                    ...
                    UIHelper.ReportTypes.UnpuntualEmployeeSummary.ToString(),
                    UIHelper.QUERY_STRING_CALLER_FORM_KEY,
                    UIHelper.PAGE_UNPUNCTUAL_EMP_SUMMARY,
        ...
        private void RestoreDataFromCollection()
        {
            ...
            if (this.UnunctualEmpStorage.ContainsKey("CallerForm"))
                this.CallerForm = UIHelper.ConvertObjectToString(this.UnunctualEmpStorage["CallerForm"]);
            else
                this.CallerForm = string.Empty;
            #endregion

            #region Restore session values
            if (this.UnunctualEmpStorage.ContainsKey("CostCenterList"))
                this.CostCenterList = this.UnunctualEmpStorage["CostCenterList"] as List<CostCenterEntity>;
            else
                this.CostCenterList = null;

            if (this.UnunctualEmpStorage.ContainsKey("ReportDataList"))
                this.ReportDataList = this.UnunctualEmpStorage["ReportDataList"] as List<PunctualityEntity>;
            else
                this.ReportDataList = null;

            if (this.UnunctualEmpStorage.ContainsKey("EmployeeNo"))
                this.EmployeeNo = UIHelper.ConvertObjectToInt(this.UnunctualEmpStorage["EmployeeNo"]);
            else
                this.EmployeeNo = 0;
            ...
            if (this.UnunctualEmpStorage.ContainsKey("chkPayPeriod"))
                this.chkPayPeriod.Checked = UIHelper.ConvertObjectToBolean(this.UnunctualEmpStorage["chkPayPeriod"]);
            else
                this.chkPayPeriod.Checked = false;

            this.chkPayPeriod_CheckedChanged(this.chkPayPeriod, new EventArgs());

            if (this.UnunctualEmpStorage.ContainsKey("txtEmpNo"))
                this.txtEmpNo.Text = UIHelper.ConvertObjectToString(this.UnunctualEmpStorage["txtEmpNo"]);
            else
                this.txtEmpNo.Text = string.Empty;

            if (this.UnunctualEmpStorage.ContainsKey("txtYear"))
                this.txtYear.Text = UIHelper.ConvertObjectToString(this.UnunctualEmpStorage["txtYear"]);
            else
                this.txtYear.Text = string.Empty;

            if (this.UnunctualEmpStorage.ContainsKey("cboMonth"))
            {
                this.cboMonth.SelectedValue = UIHelper.ConvertObjectToString(this.UnunctualEmpStorage["cboMonth"]);
            }
            else
            {
                this.cboMonth.Text = string.Empty;
            ...
            if (this.UnunctualEmpStorage.ContainsKey("cboCostCenter"))
                this.cboCostCenter.SelectedValue = UIHelper.ConvertObjectToString(this.UnunctualEmpStorage["cboCostCenter"]);
            else
            {
                this.cboCostCenter.Text = string.Empty;
                this.cboCostCenter.SelectedIndex = -1;
            ...
            if (this.UnunctualEmpStorage.ContainsKey("dtpDateFrom"))
                this.dtpDateFrom.SelectedDate = UIHelper.ConvertObjectToDate(this.UnunctualEmpStorage["dtpDateFrom"]);
            else
                this.dtpDateFrom.SelectedDate = null;

            if (this.UnunctualEmpStorage.ContainsKey("dtpDateTo"))
                this.dtpDateTo.SelectedDate = UIHelper.ConvertObjectToDate(this.UnunctualEmpStorage["dtpDateTo"]);
            else
                this.dtpDateTo.SelectedDate = null;
            #endregion
        }
        ...
        {
            this.UnunctualEmpStorage.Clear();
            this.UnunctualEmpStorage.Add("FormFlag", formFlag.ToString());

            #region Save control values to session
            this.UnunctualEmpStorage.Add("txtEmpNo", this.txtEmpNo.Text.Trim());
            this.UnunctualEmpStorage.Add("txtYear", this.txtYear.Text.Trim());
            this.UnunctualEmpStorage.Add("cboMonth", this.cboMonth.SelectedValue);
            this.UnunctualEmpStorage.Add("cboCostCenter", this.cboCostCenter.SelectedValue);
            this.UnunctualEmpStorage.Add("dtpDateFrom", this.dtpDateFrom.SelectedDate);
            this.UnunctualEmpStorage.Add("dtpDateTo", this.dtpDateTo.SelectedDate);
            this.UnunctualEmpStorage.Add("chkPayPeriod", this.chkPayPeriod.Checked);
            #endregion

            #region Save Query String values to collection
            this.UnunctualEmpStorage.Add("CallerForm", this.CallerForm);
            #endregion

            #region Store session data to collection
            this.UnunctualEmpStorage.Add("CostCenterList", this.CostCenterList);
            this.UnunctualEmpStorage.Add("ReportDataList", this.ReportDataList);
            this.UnunctualEmpStorage.Add("EmployeeNo", this.EmployeeNo);
            #endregion
        }
        ...
        private void GetReportData(bool reloadDataFromDB = false)
        {
            try
            {
                ...
                List<PunctualityEntity> reportData = new List<PunctualityEntity>();
                if (!reloadDataFromDB)
                {
                ...
                else
                {
                    ...
                    DALProxy proxy = new DALProxy();
                    var source = proxy.GetUnpunctualEmployeeSummary(startDate, endDate, costCenter, empNo, ref error, ref innerError);
