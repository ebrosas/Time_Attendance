...
{
    public partial class DILReportFilter : BaseWebForm, IFormExtension
    {
        #region Enumeration
        ...
        private Dictionary<string, object> DILReportStorage
        {
            get
            {
                Dictionary<string, object> list = Session["DILReportSource"] as Dictionary<string, object>;
                if (list == null)
                    Session["DILReportSource"] = list = new Dictionary<string, object>();

                return list;
            }
            set
            {
                Session["DILReportSource"] = value;
            }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                this.Master.SetPageForm(UIHelper.FormAccessCodes.DILREPORT.ToString());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                this.Master.FormTitle = UIHelper.PAGE_DIL_REPORT_TITLE;
                #endregion
                ...
                if (!GAPFunction.CheckFormAccess(this.Master.FormAccess, GAPConstants.FormAccessIndex.Retrieve))
                {
                    ...
                    {
                        Response.Redirect(String.Format("{0}?error={1}&pageName={2}", UIHelper.PAGE_ERROR, Convert.ToInt32(UIHelper.PageErrorCodes.NoAccessOnPage), UIHelper.PAGE_DIL_REPORT_TITLE), true);
                    }
                }
                #endregion
                ...
                if (this.DILReportStorage.Count > 0)
                {
                    if (this.DILReportStorage.ContainsKey("FormFlag"))
                        formFlag = UIHelper.ConvertObjectToString(this.DILReportStorage["FormFlag"]);
                }
                #endregion
                ...
                if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.GetEmployeeInfo.ToString())
                {
                    ...
                    // Clear data storage
                    Session.Remove("DILReportStorage");
                    #endregion
                }
        ...
        protected void btnShowReport_Click(object sender, EventArgs e)
        {
                ...
                this.AttendanceList.Count > 0)
            {
                ...
                (
                    ...
                    UIHelper.PAGE_DAYINLIEU_REPORT,
        ...
        private void RestoreDataFromCollection()
        {
            ...
            if (this.DILReportStorage.ContainsKey("CallerForm"))
                this.CallerForm = UIHelper.ConvertObjectToString(this.DILReportStorage["CallerForm"]);
            else
                this.CallerForm = string.Empty;
            #endregion

            #region Restore session values
            if (this.DILReportStorage.ContainsKey("AttendanceList"))
                this.AttendanceList = this.DILReportStorage["AttendanceList"] as List<EmployeeAttendanceEntity>;
            else
            ...
            if (this.DILReportStorage.ContainsKey("txtEmpNo"))
                this.txtEmpNo.Text = UIHelper.ConvertObjectToString(this.DILReportStorage["txtEmpNo"]);
            else
                this.txtEmpNo.Text = string.Empty;

            if (this.DILReportStorage.ContainsKey("cboCostCenter"))
                this.cboCostCenter.SelectedValue = UIHelper.ConvertObjectToString(this.DILReportStorage["cboCostCenter"]);
            else
            {
                this.cboCostCenter.Text = string.Empty;
                this.cboCostCenter.SelectedIndex = -1;
            ...
            if (this.DILReportStorage.ContainsKey("dtpStartDate"))
                this.dtpStartDate.SelectedDate = UIHelper.ConvertObjectToDate(this.DILReportStorage["dtpStartDate"]);
            else
                this.dtpStartDate.SelectedDate = null;

            if (this.DILReportStorage.ContainsKey("dtpEndDate"))
                this.dtpEndDate.SelectedDate = UIHelper.ConvertObjectToDate(this.DILReportStorage["dtpEndDate"]);
            else
                this.dtpEndDate.SelectedDate = null;
            #endregion
        }
        ...
        {
            this.DILReportStorage.Clear();
            this.DILReportStorage.Add("FormFlag", formFlag.ToString());

            #region Save control values to session
            this.DILReportStorage.Add("txtEmpNo", this.txtEmpNo.Text.Trim());
            this.DILReportStorage.Add("cboCostCenter", this.cboCostCenter.SelectedValue);
            this.DILReportStorage.Add("dtpStartDate", this.dtpStartDate.SelectedDate);
            this.DILReportStorage.Add("dtpEndDate", this.dtpEndDate.SelectedDate);
            #endregion

            #region Save Query String values to collection
            this.DILReportStorage.Add("CallerForm", this.CallerForm);
            #endregion

            #region Store session data to collection
            this.DILReportStorage.Add("AttendanceList", this.AttendanceList);
            #endregion
        }
