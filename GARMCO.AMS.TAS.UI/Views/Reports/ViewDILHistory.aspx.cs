...
{
    public partial class ViewDILHistory : BaseWebForm, IFormExtension
    {
        #region Enumeration
        ...
        private Dictionary<string, object> ViewDILHistoryStorage
        {
            get
            {
                Dictionary<string, object> list = Session["ViewDILHistoryStorage"] as Dictionary<string, object>;
                if (list == null)
                    Session["ViewDILHistoryStorage"] = list = new Dictionary<string, object>();

                return list;
            }
            set
            {
                Session["ViewDILHistoryStorage"] = value;
            }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                this.Master.SetPageForm(UIHelper.FormAccessCodes.VIEWDILHIS.ToString());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                this.Master.FormTitle = UIHelper.PAGE_VIEW_DIL_HISTORY_HISTORY_TITLE;
                #endregion
                ...
                if (!GAPFunction.CheckFormAccess(this.Master.FormAccess, GAPConstants.FormAccessIndex.Retrieve))
                {
                    ...
                    {
                        Response.Redirect(String.Format("{0}?error={1}&pageName={2}", UIHelper.PAGE_ERROR, Convert.ToInt32(UIHelper.PageErrorCodes.NoAccessOnPage), UIHelper.PAGE_VIEW_DIL_HISTORY_HISTORY_TITLE), true);
                    }
                }
                #endregion
                ...
                if (this.ViewDILHistoryStorage.Count > 0)
                {
                    if (this.ViewDILHistoryStorage.ContainsKey("FormFlag"))
                        formFlag = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["FormFlag"]);
                }
                #endregion
                ...
                if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.GetEmployeeInfo.ToString())
                {
                    ...
                    // Clear data storage
                    Session.Remove("ViewDILHistoryStorage");
                    #endregion
                }
                else if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.ShowReport.ToString())
                {
                    ...
                    // Clear data storage
                    this.ViewDILHistoryStorage.Clear();
                    #endregion
                }
                else
                {
                    ...
                    this.litEmpName.Text = UIHelper.ConvertObjectToString(Session[UIHelper.GARMCO_FULLNAME]);
                    this.litPosition.Text = UIHelper.ConvertObjectToString(Session[UIHelper.GARMCO_USER_POSITION_DESC]);
                    this.litCostCenter.Text = string.Format("{0} - {1}",
                        UIHelper.ConvertObjectToString(Session[UIHelper.GARMCO_USER_COST_CENTER]),
                        UIHelper.ConvertObjectToString(Session[UIHelper.GARMCO_USER_COST_CENTER_NAME]));

                    // Enable employee search if user has allowed cost center 
                    this.btnFindEmployee.Enabled = this.AllowedCostCenterList.Count > 0;
                    #endregion
                }
            }
        ...
        protected void btnShowReport_Click(object sender, EventArgs e)
        {
                ...
                this.AttendanceList.Count > 0)
            {
                ...
                Session["DILReportSource"] = this.AttendanceList;
                ...
                (
                    ...
                    UIHelper.ReportTypes.DayInLieuReport.ToString(),
                    UIHelper.QUERY_STRING_CALLER_FORM_KEY,
                    UIHelper.PAGE_VIEW_DIL_HISTORY,
        ...
        private void RestoreDataFromCollection()
        {
            ...
            if (this.ViewDILHistoryStorage.ContainsKey("CallerForm"))
                this.CallerForm = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["CallerForm"]);
            else
                this.CallerForm = string.Empty;

            // Restore View Type
            if (this.ViewDILHistoryStorage.ContainsKey("CurrentViewType"))
            {
                string viewType = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["CurrentViewType"]);
                if (viewType != string.Empty)
                {
                    ViewType filterOption = ViewType.NotDefined;
                    try
            ...
            if (this.ViewDILHistoryStorage.ContainsKey("AttendanceList"))
                this.AttendanceList = this.ViewDILHistoryStorage["AttendanceList"] as List<EmployeeAttendanceEntity>;
            else
            ...
            if (this.ViewDILHistoryStorage.ContainsKey("txtEmpNo"))
                this.txtEmpNo.Text = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["txtEmpNo"]);
            else
                this.txtEmpNo.Text = string.Empty;

            if (this.ViewDILHistoryStorage.ContainsKey("litEmpName"))
                this.litEmpName.Text = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["litEmpName"]);
            else
                this.litEmpName.Text = string.Empty;

            if (this.ViewDILHistoryStorage.ContainsKey("litPosition"))
                this.litPosition.Text = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["litPosition"]);
            else
                this.litPosition.Text = string.Empty;

            if (this.ViewDILHistoryStorage.ContainsKey("litCostCenter"))
                this.litCostCenter.Text = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["litCostCenter"]);
            else
                this.litCostCenter.Text = string.Empty;

            if (this.ViewDILHistoryStorage.ContainsKey("txtYear"))
                this.txtYear.Text = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["txtYear"]);
            else
                this.txtYear.Text = string.Empty;

            if (this.ViewDILHistoryStorage.ContainsKey("dtpStartDate"))
                this.dtpStartDate.SelectedDate = UIHelper.ConvertObjectToDate(this.ViewDILHistoryStorage["dtpStartDate"]);
            else
                this.dtpStartDate.SelectedDate = null;

            if (this.ViewDILHistoryStorage.ContainsKey("dtpEndDate"))
                this.dtpEndDate.SelectedDate = UIHelper.ConvertObjectToDate(this.ViewDILHistoryStorage["dtpEndDate"]);
            else
                this.dtpEndDate.SelectedDate = null;

            if (this.ViewDILHistoryStorage.ContainsKey("cboMonth"))
                this.cboMonth.SelectedValue = UIHelper.ConvertObjectToString(this.ViewDILHistoryStorage["cboMonth"]);
            else
            {
                this.cboMonth.Text = string.Empty;
                this.cboMonth.SelectedIndex = -1;
            ...
            if (this.ViewDILHistoryStorage.ContainsKey("chkPayPeriod"))
                this.chkPayPeriod.Checked = UIHelper.ConvertObjectToBolean(this.ViewDILHistoryStorage["chkPayPeriod"]);
            else
                this.chkPayPeriod.Checked = false;
        ...
        {
            this.ViewDILHistoryStorage.Clear();
            this.ViewDILHistoryStorage.Add("FormFlag", formFlag.ToString());

            #region Save control values to session
            this.ViewDILHistoryStorage.Add("txtEmpNo", this.txtEmpNo.Text.Trim());
            this.ViewDILHistoryStorage.Add("litEmpName", this.litEmpName.Text.Trim());
            this.ViewDILHistoryStorage.Add("litPosition", this.litPosition.Text.Trim());
            this.ViewDILHistoryStorage.Add("litCostCenter", this.litCostCenter.Text.Trim());
            this.ViewDILHistoryStorage.Add("txtYear", this.txtYear.Text.Trim());
            this.ViewDILHistoryStorage.Add("cboMonth", this.cboMonth.SelectedValue);
            this.ViewDILHistoryStorage.Add("dtpStartDate", this.dtpStartDate.SelectedDate);
            this.ViewDILHistoryStorage.Add("dtpEndDate", this.dtpEndDate.SelectedDate);
            this.ViewDILHistoryStorage.Add("chkPayPeriod", this.chkPayPeriod.Checked);            
            #endregion

            #region Save Query String values to collection
            this.ViewDILHistoryStorage.Add("CallerForm", this.CallerForm);
            this.ViewDILHistoryStorage.Add("CurrentViewType", this.CurrentViewType);
            #endregion

            #region Store session data to collection
            this.ViewDILHistoryStorage.Add("AttendanceList", this.AttendanceList);
            #endregion
        }
        ...
        private void GetDILHistory(bool reloadDataFromDB = false)
        {
            try
            {
                ...
                else
                {
                    ...
                    var source = proxy.GetDILReportData(empNo, costCenter, startDate, endDate, ref error, ref innerError);
