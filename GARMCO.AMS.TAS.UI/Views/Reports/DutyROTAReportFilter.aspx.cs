...
{
    public partial class DutyROTAReportFilter : BaseWebForm, IFormExtension
    {
        #region Enumeration
        ...
        private Dictionary<string, object> DutyROTAReportStorage
        {
            get
            {
                Dictionary<string, object> list = Session["DutyROTAReportStorage"] as Dictionary<string, object>;
                if (list == null)
                    Session["DutyROTAReportStorage"] = list = new Dictionary<string, object>();

                return list;
            }
            set
            {
                Session["DutyROTAReportStorage"] = value;
            }
        }
        ...
        private List<DutyROTAEntity> ReportDataList
        {
            get
            {
                List<DutyROTAEntity> list = ViewState["ReportDataList"] as List<DutyROTAEntity>;
                if (list == null)
                    ViewState["ReportDataList"] = list = new List<DutyROTAEntity>();
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                this.Master.SetPageForm(UIHelper.FormAccessCodes.ROTAREPORT.ToString());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                this.Master.FormTitle = UIHelper.PAGE_DUTY_ROTA_REPORT_TITLE;
                #endregion
                ...
                if (!GAPFunction.CheckFormAccess(this.Master.FormAccess, GAPConstants.FormAccessIndex.Retrieve))
                {
                    ...
                    {
                        Response.Redirect(String.Format("{0}?error={1}&pageName={2}", UIHelper.PAGE_ERROR, Convert.ToInt32(UIHelper.PageErrorCodes.NoAccessOnPage), UIHelper.PAGE_DUTY_ROTA_REPORT_TITLE), true);
                    }
                }
                #endregion
                ...
                if (this.DutyROTAReportStorage.Count > 0)
                {
                    if (this.DutyROTAReportStorage.ContainsKey("FormFlag"))
                        formFlag = UIHelper.ConvertObjectToString(this.DutyROTAReportStorage["FormFlag"]);
                }
                #endregion
                ...
                if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.GetEmployeeInfo.ToString())
                {
                    ...
                    // Clear data storage
                    Session.Remove("DutyROTAReportStorage");
                    #endregion
                }
                else if (formFlag != string.Empty && formFlag == UIHelper.PagePostBackFlags.ShowReport.ToString())
                {
                    ...
                    // Clear data storage
                    this.DutyROTAReportStorage.Clear();
                    #endregion
                }
                else
                {
        ...
        protected void btnShowReport_Click(object sender, EventArgs e)
        {
            ...
            GetDutyROTAReportData(true);

            #region Show the Duty ROTA Report
                ...
                this.ReportDataList.Count > 0)
            {
                ...
                Session["DutyROTAReportSource"] = this.ReportDataList;
                ...
                (
                    ...
                    UIHelper.PAGE_DUTY_ROTA_REPORT,
        ...
        protected void lbCostCenter_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void RestoreDataFromCollection()
        {
            ...
            if (this.DutyROTAReportStorage.ContainsKey("CallerForm"))
                this.CallerForm = UIHelper.ConvertObjectToString(this.DutyROTAReportStorage["CallerForm"]);
            else
                this.CallerForm = string.Empty;

            if (this.DutyROTAReportStorage.ContainsKey("CurrentViewType"))
            {
                string viewType = UIHelper.ConvertObjectToString(this.DutyROTAReportStorage.ContainsKey("CurrentViewType"));
                if (viewType != string.Empty)
                {
                    ViewType filterOption = ViewType.NotDefined;
                    try
            ...
            if (this.DutyROTAReportStorage.ContainsKey("ReportDataList"))
                this.ReportDataList = this.DutyROTAReportStorage["ReportDataList"] as List<DutyROTAEntity>;
            else
                this.ReportDataList = null;

            if (this.DutyROTAReportStorage.ContainsKey("CostCenterList"))
                this.CostCenterList = this.DutyROTAReportStorage["CostCenterList"] as List<CostCenterEntity>;
            else
            ...
            if (this.DutyROTAReportStorage.ContainsKey("txtEmpNo"))
                this.txtEmpNo.Text = UIHelper.ConvertObjectToString(this.DutyROTAReportStorage["txtEmpNo"]);
            else
                this.txtEmpNo.Text = string.Empty;

            if (this.DutyROTAReportStorage.ContainsKey("dtpStartDate"))
                this.dtpStartDate.SelectedDate = UIHelper.ConvertObjectToDate(this.DutyROTAReportStorage["dtpStartDate"]);
            else
                this.dtpStartDate.SelectedDate = null;

            if (this.DutyROTAReportStorage.ContainsKey("dtpEndDate"))
                this.dtpEndDate.SelectedDate = UIHelper.ConvertObjectToDate(this.DutyROTAReportStorage["dtpEndDate"]);
            else
                this.dtpEndDate.SelectedDate = null;

            if (this.DutyROTAReportStorage.ContainsKey("lbCostCenter"))
            {
                if (this.lbCostCenter.Items.Count > 0)
                {
                    dynamic selectedItems = this.DutyROTAReportStorage["lbCostCenter"];
                    //List<RadListBoxItem> selectedItems = this.DutyROTAReportStorage["lbCostCenter"] as List<RadListBoxItem>;
            ...
            if (this.DutyROTAReportStorage.ContainsKey("chkSelectAll"))
                this.chkSelectAll.Checked = UIHelper.ConvertObjectToBolean(this.DutyROTAReportStorage["chkSelectAll"]);
            else
        ...
        {
            this.DutyROTAReportStorage.Clear();
            this.DutyROTAReportStorage.Add("FormFlag", formFlag.ToString());

            #region Save control values to session
            this.DutyROTAReportStorage.Add("txtEmpNo", this.txtEmpNo.Text.Trim());
            this.DutyROTAReportStorage.Add("lbCostCenter", this.lbCostCenter.CheckedItems);
            this.DutyROTAReportStorage.Add("dtpStartDate", this.dtpStartDate.SelectedDate);
            this.DutyROTAReportStorage.Add("dtpEndDate", this.dtpEndDate.SelectedDate);
            this.DutyROTAReportStorage.Add("chkSelectAll", this.chkSelectAll.Checked);
            #endregion

            #region Save Query String values to collection
            this.DutyROTAReportStorage.Add("CallerForm", this.CallerForm);
            this.DutyROTAReportStorage.Add("CurrentViewType", this.CurrentViewType);
            #endregion

            #region Store session data to collection
            this.DutyROTAReportStorage.Add("ReportDataList", this.ReportDataList);
            this.DutyROTAReportStorage.Add("CostCenterList", this.CostCenterList);
            #endregion
        }
        ...
        private void GetDutyROTAReportData(bool reloadDataFromDB = false)
        {
            try
            {
                ...
                List<DutyROTAEntity> reportDataList = new List<DutyROTAEntity>();
                if (!reloadDataFromDB)
                {
                ...
                else
                {
                    ...
                    DALProxy proxy = new DALProxy();
                    var source = proxy.GetDutyROTAReportData(startDate, endDate, costCenterList, empNo, ref error, ref innerError);
